...
export default function ChatInterface({ alertaId, isEstudante, onClose }: ChatInterfaceProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/80 backdrop-blur-xl border-l border-white/10 shadow-2xl relative">
        ...
        {mensagens.length === 0 ? (
          ...
          mensagens.map((msg) => {
            ...
            return (
              <div 
                key={msg.id} 
                className={`flex max-w-[85%] ${isMine ? 'self-end bg-primary/20 backdrop-blur-sm text-white rounded-l-xl rounded-tr-xl border border-primary/30 shadow-lg shadow-primary/10' : 'self-start bg-white/10 backdrop-blur-sm text-slate-200 rounded-r-xl rounded-tl-xl border border-white/10 shadow-lg'}`}
              >
      ...
      <form onSubmit={handleSendMessage} className="p-4 border-t border-white/10 bg-black/30 backdrop-blur-md">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-primary hover:bg-primary-hover text-white p-3 rounded-xl flex items-center justify-center transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-primary/20"
          >
            <Send size={18} />
          </button>
        </div>
